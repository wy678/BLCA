...
library(circlize)
library(Hmisc)
setwd("x:\\xxxx\\xxxx\\xxx")

load("x:/xxxx/xxxx/xxx/pain_exp_BLCA.rda")
load("x:/xxxx/xxxx/xxx/cox_pain.rda")
load("x:/xxxx/xxxx/xxxx/risk_data.rda")
load("x:/xxxx/xxxx/TCGA_BLCA.rda")
...
sig<-rownames(cox_pain[which(cox_pain[,6]<0.05),])
exp<-pain_exp_BLCA[sig,]
...
NES_Cluster1<-exp[,Cluster11]
NES_Cluster2<-exp[,Cluster22]
...
row.names(random_wilcox)<-row.names(exp)
...
dat_cluster<-cbind(exp[,Cluster11],exp[,Cluster22])
...
pdf("BLCA_EXP17_heatmap.pdf",,width = 10,height = 8)
draw(ha_cn1A)
dev.off()
