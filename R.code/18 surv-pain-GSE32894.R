...
GSE1<-Bladder$GSE32894
exp<-exprs(GSE1)
pain<-read.gmt("xx\\xxxx\\xxxx\\xxxx\\xxxx\\xxx\\xxxx\\GOBP_SENSORY_PERCEPTION_OF_PAIN.v2022.1.Hs.gmt")
...
pain_exp<-pain_exp[-which(rownames(pain_exp) ==  "5229"),]
rownames(pain_exp)<-pain_exp$gene
pain_exp_GSE32894<-pain_exp[,-which(colnames(pain_exp) == "gene")]

save(pain_exp_GSE32894,file = "pain_exp_GSE32894.rda")
...
pain_exp_GSE32894<-t(pain_exp_GSE32894)
pain_exp_GSE32894<-data.frame(pain_exp_GSE32894)
pain_exp_GSE32894<-pain_exp_GSE32894[rownames(pdata),]
coxdata<-cbind(pdata,pain_exp_GSE32894)
...
cox_pain_GSE32894<-na.omit(cox_pain)
colnames(cox_pain_GSE32894)<-c("HR","Lower.95","Upper.95","Logtest","Logrank","p_value")
save(cox_pain_GSE32894,file = "cox_pain_GSE32894.rda")

cox_pain_GSE32894<-data.frame(cox_pain_GSE32894[which(cox_pain_GSE32894[,6]<0.05),])

pain_exp_GSE32894<-t(pain_exp_GSE32894)

pain_exp_GSE32894_1<-as.matrix(pain_exp_GSE32894[rownames(cox_pain_GSE32894),])

data<-matrix(NA,nrow(pain_exp_GSE32894_1),ncol(pain_exp_GSE32894_1))
colnames(data)<-colnames(pain_exp_GSE32894_1)
rownames(data)<-rownames(pain_exp_GSE32894_1)

for (k in 1:nrow(pain_exp_GSE32894_1)) {
  data[k,]<-pain_exp_GSE32894_1[k,]*log(cox_pain_GSE32894[k,1])
}
...
p1=ggsurvplot(fit1,legend.title="Subtype",
              ...
              title=paste0("Kaplan-Meier Curve for GSE32894"),
...
pdf("GSE32894_pain_sur.pdf",width = 8,height = 6)
p4
dev.off()
